...
import "./MyCoursesMember.css"; // Member-specific styles
...
const MyCourses = () => {
        ...
        const fetchCourses = async () => {
            ...
            try {
                ...
                const currentResponse = await axios.get(
                    `http://127.0.0.1:8000/api/member/current_courses/?user_id=${userId}`
                );

                // Fetch previous courses
                const previousResponse = await axios.get(
                    `http://127.0.0.1:8000/api/member/previous_courses/?user_id=${userId}`
                );
    ...
    const handleViewClick = async (courseId) => {
        try {
            const response = await axios.get(
                `http://127.0.0.1:8000/api/member/get_course/?course_id=${courseId}`
            );
    ...
    const handleWithdraw = async () => {
        ...
        try {
            const userId = localStorage.getItem("memberId");
            const response = await axios.post("http://127.0.0.1:8000/api/member/withdraw_course/", {
                swimmer_id: userId,
            ...
            if (response.status === 200) {
                alert(
                    `Course "${selectedCourse.course_name}" has been withdrawn.\nReason: ${terminationReason}`
                );
                handleCloseModal();
            } else {
                    ...
                    <div className="category-selector">
                        <button
                            className={`category-btn ${
                                selectedCategory === "current" ? "active" : ""
                            }`}
                        ...
                        <button
                            className={`category-btn ${
                                selectedCategory === "previous" ? "active" : ""
                            }`}
                            ...
                            coursesToDisplay.map((course) => (
                                <div key={course.course_id} className="course-card">
                                    ...
                                    <div className="course-details">
                                        ...
                                        <p><strong>Price:</strong> {course.price} TL</p>
                                    </div>
            ...
            {isModalOpen && selectedCourse && (
                <div className="modal-overlay" onClick={handleCloseModal}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <p><strong>Price:</strong> {selectedCourse.price} TL</p>
