...
import Sidebar from "./LayoutCoach.jsx";
...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            const userId = localStorage.getItem("coachId"); // Get user ID from local storage
            if (!userId) {
            ...
            try {
                const response = await axios.get(
                    `http://127.0.0.1:8000/api/get_user/?user_id=${userId}`
                );
    ...
    const sendUpdateRequest = async (updateData) => {
        try {
            await axios.post(
                "http://127.0.0.1:8000/api/update_coach_profile/",
                updateData
            );
